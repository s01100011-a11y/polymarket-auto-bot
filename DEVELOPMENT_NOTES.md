...
- Added a per-signal **BUY LIVE** button for CFB queued-preview and stale signals in the dashboard.
...
- Signal state tracks the manual request separately from the preview/stale classification so the dashboard can show PENDING/LEASED/DONE/FAILED without converting the CFB worker to automatic trading.
