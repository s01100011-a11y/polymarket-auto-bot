...
<div class="wrap">
  ...
  <section class="panel active" id="current">
    ...
    <div class="exec-status-box">
      <div class="exec-status-head"><div><div class="label">Executor status</div><div class="note" style="margin:3px 0 0">Recent BUY/SELL confirmations and failures</div></div><button class="tab" type="button" id="refreshExecutorEvents">Refresh status</button></div>
      <div class="exec-events" id="executorEvents"><div class="muted">Loading executor status…</div></div>
    </div>
...
function executorStateClass(s){return s==='DONE'?'ok':s==='FAILED'?'fail':'run'}
function executorStateLabel(s){return s==='DONE'?'CONFIRMED':s==='FAILED'?'FAILED':s==='LEASED'?'EXECUTING':'QUEUED'}
async function loadExecutorEvents(){
 const box=document.getElementById('executorEvents');if(!box)return;
 try{
  const r=await fetch('/api/executor/recent-actions?limit=12',{cache:'no-store'}),d=await r.json();
  if(!r.ok)throw new Error(d.detail||'Status request failed');
  const events=(d.events||[]).filter(x=>['BUY','SELL'].includes(x.action));
  box.innerHTML=events.length?events.map(x=>`<div class="exec-event ${executorStateClass(x.status)}"><span class="status">${esc(x.action||'—')}</span><span class="evstate">${executorStateLabel(x.status)}</span><span class="evmsg">${esc(x.message||x.error||'—')}</span><span class="evtime">${when(x.updated_at||x.created_at)}</span></div>`).join(''):'<div class="muted">No recent executor actions.</div>';
 }catch(e){box.innerHTML='<div class="exec-event fail"><span class="status">STATUS</span><span class="evstate">FAILED</span><span class="evmsg">'+esc(String(e))+'</span><span></span></div>'}
}
async function load(){
 try{
  ...
  const lr=d.live_trades.map(x=>{const canSell=!x.paper&&['termux_executor','slack_live'].includes(x.source);return `<tr><td><span class="status">LIVE</span></td><td class="market">${link(x.market_url,x.market)}</td><td>${esc(x.outcome||'—')}</td><td>${price(x.entry_price)}</td><td>${price(x.limit_price)}</td><td>${price(x.current_midpoint)}</td><td>${x.shares===null||x.shares===undefined?'—':Number(x.shares).toFixed(4)}</td><td>${money(x.budget_usdc)}</td><td>${money(x.current_value_usdc)}</td><td class="${Number(x.estimated_pnl)>0?'green':Number(x.estimated_pnl)<0?'red':''}">${money(x.estimated_pnl)} ${x.pnl_percent===null||x.pnl_percent===undefined?'':('('+Number(x.pnl_percent).toFixed(1)+'%)')}</td><td class="muted">${esc(x.entry_source||'—')}</td><td>${when(x.submitted_at)}</td><td>${canSell?'<button class="sell-btn" data-sell-trade="'+esc(x.id)+'" data-sell-shares="'+esc(x.shares||'')+'">SELL</button>':'—'}</td></tr>`});
  document.getElementById('currentBody').innerHTML=(d.watching.length?'<div class="label" style="margin:8px 0">Price watches</div>':'')+table(['Status','Outcome / market','Type','Current','Limit','Budget','Expires','Error'],wr)+(d.live_trades.length?'<div class="label" style="margin:18px 0 8px">Submitted live trades</div>':'')+table(['Status','Market','Outcome','Actual entry','Limit','Current','Shares','Actual cost','Current value','Live P/L','Source','Submitted','Action'],lr);
...
async function waitForSellRequest(requestId,timeoutMs=90000){
 ...
 while(Date.now()-started<timeoutMs){
  const r=await fetch('/api/executor/request-status/'+encodeURIComponent(requestId),{cache:'no-store'}),d=await r.json();
  if(!r.ok)throw new Error(d.detail||'SELL status check failed');
  if(d.status==='DONE')return d.result||{};
  if(d.status==='FAILED')throw new Error(d.error||'SELL failed');
  await new Promise(res=>setTimeout(res,700));
 }
 throw new Error('Timed out waiting for SELL execution');
}
document.addEventListener('click',async ev=>{
 const btn=ev.target.closest('[data-sell-trade]');if(!btn)return;
 const tradeId=btn.getAttribute('data-sell-trade'),shares=btn.getAttribute('data-sell-shares');
 if(!tradeId)return;
 const shareText=shares&&Number.isFinite(Number(shares))?' '+Number(shares).toFixed(4)+' shares':' the remaining tracked shares';
 if(!confirm('SELL'+shareText+' for this LIVE Polymarket position? This queues a REAL sell through the connected executor.'))return;
 const oldText=btn.textContent;btn.disabled=true;btn.textContent='SELLING…';
 try{
  const r=await fetch('/api/executor/request-sell/'+encodeURIComponent(tradeId),{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
  const q=await r.json();if(!r.ok)throw new Error(q.detail||'Unable to queue SELL');
  await loadExecutorEvents();
  const result=await waitForSellRequest(q.request_id);
  await loadExecutorEvents();
  await load();
 }catch(err){
  await loadExecutorEvents();
...
document.getElementById('refreshExecutorEvents').onclick=loadExecutorEvents;
