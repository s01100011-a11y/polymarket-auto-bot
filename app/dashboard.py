...
document.addEventListener('click',async ev=>{
 ...
 try{
  ...
  alert('SELL completed'+(result.sell_price?' at '+result.sell_price:'')+(result.realized_pnl!==undefined?' · P/L '+result.realized_pnl:''));
  await load();
 }catch(err){
  alert('SELL failed: '+err.message);btn.disabled=false;btn.textContent=oldText;
 }
});
