#!/usr/bin/env python3
"""Bootstrap pw_export_{league}.jsonl from game_history.

Reads game_history_{league}.json and writes one JSONL line per non-suppressed
PW call with game context merged.  Safe to re-run — overwrites the output file.

Usage:
    python3 export_pw_jsonl.py                          # NBA (default)
    NBA_MONITOR_CONFIG=config-wnba.json python3 export_pw_jsonl.py  # WNBA
...
def main():
    ...
    out_path = league_config.state_path("pw_export.jsonl")
    total = 0
    skipped_suppressed = 0

    # Import outcomes for _game_season_year
    try:
        import outcomes
        season_year = outcomes._game_season_year
    except Exception:
        season_year = lambda r: ""  # noqa: E731
    ...
    with open(out_path, "w") as out:
        for rec in history:
            ...
            game_ctx = {
                ...
                "game_date": game_date,
                "home_team": str(rec.get("home_name") or rec.get("home_abbr", "")),
                "away_team": str(rec.get("away_name") or rec.get("away_abbr", "")),
                "home_id": str(rec.get("home_id", "")),
                "away_id": str(rec.get("away_id", "")),
                "home_abbr": str(rec.get("home_abbr", "")),
                "away_abbr": str(rec.get("away_abbr", "")),
                "home_score": rec.get("home_score"),
                "away_score": rec.get("away_score"),
                "winner_id": str(rec.get("winner_id", "")),
                "spread_fav_id": str(rec.get("spread_fav_id", "")),
                "season": season_year(rec),
